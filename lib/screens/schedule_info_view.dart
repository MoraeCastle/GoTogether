...
class _ScheduleInfoView extends State<ScheduleInfoView> {
  ...
  Future<void> setSelectDay(DateTime dateTime) async {
    ...
    // 필요없지만 다른 씬에서 프로바이더 없이 사용할 경우를 대비해서.
            ...
            child: Column(
              children: [
                        ...
                        // Consumer를 활용해서 provider에 접근하여 데이터를 받아올 수 있다
                          builder: (context, provider, child) {
                            ...
                            return Text(
                              ...
                              SystemUtil.changePrintDate(provider.travel
                                  .getDate()) + " (" +
                                  SystemUtil.getTravelDay(
                                      provider.travel.getDate()).toString() +
                                  "일)", // count를 화면에 출력
                ...
                IconButton(
                  icon: Icon(Icons.add),
                  onPressed: () {
                    BotToast.showText(text: context.read<ScheduleClass>().selectDate.toString());
                  },
                ),
                Container(
                  padding: EdgeInsets.all(15),
                  ...
                  child: TableCalendar(
                    locale: 'ko_KR',
                    rangeStartDay: context.watch<ScheduleClass>().getDateTime(
                        0),
                    ...
                    selectedDayPredicate: (day) {
                      return isSameDay(context.watch<ScheduleClass>().selectDate, day);
                    },
                    onDaySelected: (selectedDay, focusedDay) {
                      // BotToast.showText(text: context.read<ScheduleClass>().travel.getDate());
                      if (SystemUtil.isDateInSchedule(context
                          .read<ScheduleClass>()
                          .travel
                          .getDate(), selectedDay)) {
                      ...
                      Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(
                                  width: 500,
                                  child: Divider(color: Color.fromARGB(
                                      50, 0, 0, 0), thickness: 1.0)),
                              Container(
                                height: 400,
                                decoration: BoxDecoration(
                                    border: Border.all(
  ...
  _AppointmentDataSource _getCalendarDataSource(List<Schedule> data) {
    ...
    /*for (Schedule item in data) {
      if (item.getRouteMap().containsKey(key))
    }*/
